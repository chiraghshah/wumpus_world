...
public class CheckTrueFalse {
	...
	public static void main(String[] args) {
		...
		LogicalExpression kb_with_wumpusRules = new LogicalExpression();

		LogicalExpression statement1 = new LogicalExpression();
		LogicalExpression notOfStatment1 = new LogicalExpression();
		
		
		TTEntailsAlgorithm ttInstance = new TTEntailsAlgorithm();
		
		TTEntailsAlgorithm.Model m = ttInstance.new Model();
		try {
			inputStream = new BufferedReader(new FileReader(args[0]));
			kb_with_wumpusRules.setConnective("and");

			while ((buffer = inputStream.readLine()) != null) {
				if (!(buffer.startsWith("#") || (buffer.equals("")))) {
					LogicalExpression subExpression = readExpression(buffer);
					kb_with_wumpusRules.setSubexpression(subExpression);
		...
		if (!valid_expression(kb_with_wumpusRules)) {
			System.out.println("invalid knowledge base");
			exit_function(0);
		}
		String alpha1 = "";//statement without negation..CS
		String notOfalpha1 = "";//statement with negation..
		try {
			inputStream = new BufferedReader(new FileReader(args[2]));
			getSymbols(kb_with_wumpusRules);
			...
			while ((buffer = inputStream.readLine()) != null) {
				if (!buffer.startsWith("#")) {
					if (buffer.contains("not")) {
						...
						notOfalpha1 = split[1].substring(split[1].length() - 1);
					} else {
						alpha1 = buffer;
						notOfalpha1 = "(not " + buffer + ")";
					}

					statement1 = readExpression(alpha1);
					notOfStatment1 = readExpression(notOfalpha1);
		...
		boolean output1 = ttInstance.ttEntails(kb_with_wumpusRules, statement1, m);
		boolean output2 = ttInstance.ttEntails(kb_with_wumpusRules, notOfStatment1, m);

		showOutput(output1, output2);
	...
	private static void showOutput(boolean output1, boolean output2) {
		try {
